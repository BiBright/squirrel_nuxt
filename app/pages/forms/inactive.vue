<template>
  <div class="container">
    <div class="row">
      <div class="col-12">
        <AppBreadcrumb :items="[{ label: 'Forms', to: '/forms' }, { label: 'Inactive Forms' }]" />
      </div>

      <AppPageHeader title="Inactive Forms" />

      <div class="col-12">
        <div class="deleted-toolbar">
          <div class="input-search-wrap">
            <span class="material-icons-round input-search-wrap__icon">search</span>
            <input v-model="search" type="text" class="input-search" placeholder="Search forms..." />
          </div>
...
onMounted(async () => {
  try {
    // Backend endpoint needed: GET /forms/inactive
    const res = await api<{ data: DeletedItem[] }>('/forms/inactive')
