<template>
  <div class="container">
    <div class="row">
      <div class="col-12">
        <AppBreadcrumb :items="[{ label: 'Users', to: '/users' }, { label: 'Inactive Users' }]" />
      </div>

      <AppPageHeader title="Inactive Users" />

      <div class="col-12">
        <div class="deleted-toolbar">
          <div class="input-search-wrap">
            <span class="material-icons-round input-search-wrap__icon">search</span>
            <input v-model="search" type="text" class="input-search" placeholder="Search users..." />
          </div>
          ...
          <NuxtLink to="/users" class="btn-back">
      ...
      <div class="col-12">
        ...
        <AppBlankState
          ...
          message="Users that are deactivated will appear here."
        />
...
onMounted(async () => {
  try {
    // Backend endpoint needed: GET /users/inactive
    const res = await api<{ data: DeletedItem[] }>('/users/inactive')
